...
export function DashboardShell({ user, title, subtitle, children }: DashboardShellProps) {
  ...
  /* Yig'ilgan kenglik 84px: 46px tugma + 2×12px chekka + zaxira.
     `ready` bo'lmaguncha o'tish animatsiyasi o'chiq — aks holda
     sahifa har yuklanganda panel sirg'alib ochilgandek ko'rinadi. */
  const railWidth = collapsed ? "84px" : "280px";
  const motion = ready ? "transition-[width,left,padding] duration-300 ease-out" : "";

  return (
    <div
      className="kpk-rail-wrap relative min-h-screen"
      style={{ "--kpk-rail-w": railWidth } as React.CSSProperties}
    >
      <PageBackground />

      {/* ── SIDEBAR: doimiy, faqat katta ekranlarda ── */}
      <aside
        className={cn("kpk-bar fixed inset-y-0 left-0 z-30 hidden border-r lg:block", motion)}
        style={{ width: "var(--kpk-rail-w, 280px)" }}
      >
        {user ? (
      ...
      {/* ── ASOSIY USTUN ── */}
      <div
        className={cn("relative z-[1]", motion)}
        style={{ paddingLeft: "var(--kpk-rail, 0px)" }}
      >
        {/* TOPBAR */}
        <header
          className={cn(
            ...
            "border-b px-8 py-4 max-md:px-4",
            motion
          )}
          style={{ left: "var(--kpk-rail, 0px)" }}
        >
          {/* Mobil menyu tugmasi */}
          {user && (
            <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
              ...
              <SheetContent hideClose>
                ...
                <DashboardSidebar user={user} onNavigate={() => setDrawerOpen(false)} forceExpanded />
              </SheetContent>
            </Sheet>
          )}
        ...
        {/* MAIN */}
        <main className="px-8 pb-8 pt-[119px] max-md:px-4 max-md:pb-5 max-md:pt-[111px]">{children}</main>
      </div>
    </div>
  );
}